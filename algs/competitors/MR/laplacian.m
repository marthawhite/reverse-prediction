function [L,options] = laplacian(options,X,verbose)  
...
if verbose fprintf(1, 'Computing Graph Laplacian\n'); end
...
switch options.GraphWeights
	...
	case 'binary'
		 if verbose fprintf(1,'Using Binary weights\n'); end
	...
	case 'heat'     
    		 if isstr(options.GraphWeightParam) % default
    	       ...
    	       if verbose fprintf(1,['Using heat kernel of width ' num2str(t) '\n']);end
   ...
   if options.GraphNormalize==0
       ...
       if verbose fprintf(1, 'Normalizing the Graph Laplacian\n'); end
...
if verbose fprintf(1,['Graph Laplacian computation took ' num2str(toc) '  seconds.\n']);end
